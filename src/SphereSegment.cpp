...
class CSphereSegment::Surface: public osg::Drawable
{
public:
    Surface(CSphereSegment* ss): osg::Drawable(), _ss(ss) {}
...
void CSphereSegment::Surface::drawImplementation(osg::RenderInfo& renderInfo) const
{
    _ss->Surface_drawImplementation(*renderInfo.getState());
}

osg:: BoundingBox CSphereSegment::Surface::computeBound() const
{
 ...
 */
class CSphereSegment::EdgeLine: public osg::Drawable
{
public:
    EdgeLine(CSphereSegment* ss): osg::Drawable(), _ss(ss) { init(); }
...
void CSphereSegment::EdgeLine::drawImplementation(osg::RenderInfo& renderInfo) const
{
    _ss->EdgeLine_drawImplementation(*renderInfo.getState());
}

osg::BoundingBox CSphereSegment::EdgeLine::computeBound() const
{
    osg::BoundingBox bbox;
...
/**
SphereSegment::SurfaceLine is the Drawable which represents the line around the surface
of the specified area of the sphere's SurfaceLine.
*/
class CSphereSegment::SurfaceLine: public osg::Drawable
{
public:
	SurfaceLine(CSphereSegment* ss): osg::Drawable(), _ss(ss) { init(); }
	...
	SurfaceLine(const SurfaceLine& rhs, const osg::CopyOp& co=osg::CopyOp::SHALLOW_COPY):osg::Drawable(rhs,co), _ss(0)
	{
		OSG_WARN<<
			"Warning: unexpected call to osgSim::SphereSegment::EdgeLine() copy constructor"<<std::endl;
	}

	META_Object(osgSim,SurfaceLine)
...
protected:
	...
	void init()
	{
		...
		//getOrCreateStateSet()->setAttribute(new osg::Hint(GL_LINE_SMOOTH_HINT,GL_NICEST), osg::StateAttribute::ON);
	}
...
void CSphereSegment::SurfaceLine::drawImplementation(osg::RenderInfo& renderInfo) const
{
	_ss->SurfaceLine_drawImplementation(*renderInfo.getState());
}

osg::BoundingBox CSphereSegment::SurfaceLine::computeBound() const
{
	osg::BoundingBox bbox;
 ...
 */
class CSphereSegment::Side: public osg::Drawable
{
public:
    Side(CSphereSegment* ss, CSphereSegment::SideOrientation po, CSphereSegment::BoundaryAngle pa):
...
protected:
    ...
    Side():_ss(0), _planeOrientation(CSphereSegment::AZIM), _BoundaryAngle(CSphereSegment::MIN)
    {
        OSG_WARN<<
...
private:
    CSphereSegment* _ss;
    CSphereSegment::SideOrientation _planeOrientation;
    CSphereSegment::BoundaryAngle _BoundaryAngle;
};


void CSphereSegment::Side::drawImplementation(osg::RenderInfo& renderInfo) const
{
    _ss->Side_drawImplementation(*renderInfo.getState(), _planeOrientation, _BoundaryAngle);
}

osg::BoundingBox CSphereSegment::Side::computeBound() const
{
    osg::BoundingBox bbox;
 ...
 */
class CSphereSegment::Spoke: public osg::Drawable
{
public:
    Spoke(CSphereSegment* ss, CSphereSegment::BoundaryAngle azAngle, CSphereSegment::BoundaryAngle elevAngle):
...
private:
    CSphereSegment* _ss;
    CSphereSegment::BoundaryAngle _azAngle, _elevAngle;
};

void CSphereSegment::Spoke::drawImplementation(osg::RenderInfo& renderInfo) const
{
    _ss->Spoke_drawImplementation(*renderInfo.getState(), _azAngle, _elevAngle);
}

osg::BoundingBox CSphereSegment::Spoke::computeBound() const
{
    osg::BoundingBox bbox;
...
CSphereSegment::CSphereSegment(const osg::Vec3& centre, float innerradius, float outerradius, const osg::Vec3& vec, float azRange,
                float elevRange, int density, int elevDensity, int azDensity):
    osg::Geode(),
    _centre(centre), _innerRadius(innerradius), _outerRadius(outerradius),
    _density(density),
	_elevDensity(elevDensity),_azDensity(azDensity),
...
void CSphereSegment::setCentre(const osg::Vec3& c)
{
    _centre = c;
...
const osg::Vec3& CSphereSegment::getCentre() const
{
    return _centre;
}
...
float CSphereSegment::getRadius() const
{
    return _outerRadius;
}


void CSphereSegment::setArea(const osg::Vec3& v, float azRange, float elevRange)
{
    osg::Vec3 vec(v);
...
void CSphereSegment::getArea(osg::Vec3& vec, float& azRange, float& elevRange) const
{
...
void CSphereSegment::setArea(float azMin, float azMax,
...
void CSphereSegment::getArea(float &azMin, float &azMax,
...
void CSphereSegment::setDensity(int density)
{
    _density = density;
    dirtyAllDrawableDisplayLists();
}

int CSphereSegment::getDensity() const
{
    return _density;
}

void CSphereSegment::setElevDensity(int density)
{
	_elevDensity = density;
	dirtyAllDrawableDisplayLists();
}

int CSphereSegment::getElevDensity() const
{
	return _elevDensity;
}

void CSphereSegment::setAzDensity(int density)
{
	_azDensity = density;
	dirtyAllDrawableDisplayLists();
}

int CSphereSegment::getAzDensity() const
{
	return _azDensity;
}

void CSphereSegment::init()
{

	if(_azMax - _azMin < 2 * osg::PI)
	{
	...
	addDrawable(new SurfaceLine(this));//表面网格线
...
void CSphereSegment::Surface_drawImplementation(osg::State& state) const
{
    ...
    if(_drawMask & SURFACE)
    {
        ...
        // draw back side.
        if (drawBackSide && _innerRadius > 0)
        {
            for(int i=0; i+1<=_density; i++)
            {
                ...
                for (int j=0; j<=_density; j++)
                {
                    ...
                    gl.Normal3f(-x, -y, -z);
                    gl.Vertex3f(_centre.x() + _innerRadius*x,
                            _centre.y() + _innerRadius*y,
                            _centre.z() + _innerRadius*z);
        ...
        // draw front side
        if (drawFrontSide && _outerRadius > 0)
        {
            for(int i=0; i+1<=_density; i++)
            {
                // Because we're drawing quad strips, we need to work out
                ...
                for (int j=0; j<=_density; j++)
                {
                    ...
                    gl.Normal3f(x, y, z);
                    gl.Vertex3f(_centre.x() + _outerRadius*x,
                            _centre.y() + _outerRadius*y,
                            _centre.z() + _outerRadius*z);
...
bool CSphereSegment::Surface_computeBound(osg::BoundingBox& bbox) const
{
    bbox.init();
    ...
    for(int i=0; i<=_density; i++){
        ...
        for(int j=0; j<=_density; j++){
            ...
            bbox.expandBy(
                osg::Vec3(_centre.x() + _outerRadius*cos(elev)*sin(az),
                        _centre.y() + _outerRadius*cos(elev)*cos(az),
                        _centre.z() + _outerRadius*sin(elev))
            );
        }
    }
    return true;
...
void CSphereSegment::EdgeLine_drawImplementation(osg::State& state) const
{
    const float azIncr = (_azMax - _azMin)/_density;
    const float elevIncr = (_elevMax - _elevMin)/_density;
    ...
    if(_drawMask & EDGELINE)
    {
        ...
        for(i=0; i<=_density; i++)
        {
            float az = _azMin + (i*azIncr);
            gl.Vertex3f(
                _centre.x() + _outerRadius*cos(_elevMax)*sin(az),
                _centre.y() + _outerRadius*cos(_elevMax)*cos(az),
                _centre.z() + _outerRadius*sin(_elevMax));
        }
        gl.End();
        ...
        for(i=0; i<=_density; i++)
        {
            float az = _azMin + (i*azIncr);
            gl.Vertex3f(
                _centre.x() + _outerRadius*cos(_elevMin)*sin(az),
                _centre.y() + _outerRadius*cos(_elevMin)*cos(az),
                _centre.z() + _outerRadius*sin(_elevMin));
        }
        gl.End();
		...
		if(_azMax - _azMin < 2 * osg::PI)
		{
			...
			for(j=0; j<=_density; j++)
			{
				float elev = _elevMin + (j*elevIncr);
				gl.Vertex3f(
					_centre.x() + _outerRadius*cos(elev)*sin(_azMin),
					_centre.y() + _outerRadius*cos(elev)*cos(_azMin),
					_centre.z() + _outerRadius*sin(elev));
			}
			gl.End();
			...
			for(j=0; j<=_density; j++)
			{
				float elev = _elevMin + (j*elevIncr);
				gl.Vertex3f(
					_centre.x() + _outerRadius*cos(elev)*sin(_azMax),
					_centre.y() + _outerRadius*cos(elev)*cos(_azMax),
					_centre.z() + _outerRadius*sin(elev));
			}
			gl.End();
		}
...
bool CSphereSegment::EdgeLine_computeBound(osg::BoundingBox& bbox) const
{
    bbox.init();
...
void CSphereSegment::SurfaceLine_drawImplementation(osg::State& state) const
{
	const float azIncr = (_azMax - _azMin)/_density;
	const float elevIncr = (_elevMax - _elevMin)/_density;
	// Draw the surfaceline if necessary
	// ------------------------------
	if(_drawMask & SURFACELINE)
	{
		osg::GLBeginEndAdapter& gl = (state.getGLBeginEndAdapter());

		gl.Color4fv(_surfaceLineColor.ptr());	

		if(_innerRadius > 0)
		{
			// 纬线
			float elevLineIncr = (_elevMax - _elevMin)/_elevDensity;
			int i, j;
			for(j=0; j<=_elevDensity; j++)
			{
				float elev = _elevMin + (j*elevLineIncr);
				gl.Begin(GL_LINE_STRIP);
				for(i=0; i<=_density; i++)
				{
					float az = _azMin + (i*azIncr);
					gl.Vertex3f(
						_centre.x() + _innerRadius*cos(elev)*sin(az),
						_centre.y() + _innerRadius*cos(elev)*cos(az),
						_centre.z() + _innerRadius*sin(elev));
				}
				gl.End();
			}

			// 经线
			float azLineIncr = (_azMax - _azMin)/_azDensity;
			for(i=0; i<=_azDensity; i++)
			{
				float az = _azMin + (i*azLineIncr);
				gl.Begin(GL_LINE_STRIP);
				for(j=0; j<=_density; j++)
				{
					float elev = _elevMin + (j*elevIncr);
		...
		for(j=0; j<=_elevDensity; j++)
		{
			...
			for(i=0; i<=_density; i++)
			{
				float az = _azMin + (i*azIncr);
				gl.Vertex3f(
					_centre.x() + _outerRadius*cos(elev)*sin(az),
					_centre.y() + _outerRadius*cos(elev)*cos(az),
					_centre.z() + _outerRadius*sin(elev));
			}
			gl.End();
...
void CSphereSegment::Side_drawImplementation(osg::State& state,
                                CSphereSegment::SideOrientation orientation,
                                CSphereSegment::BoundaryAngle boundaryAngle) const
{
            ...
            if(orientation == AZIM)      // This is a plane at a given azimuth
            {
                ...
                if (drawBackSide)
                {
					...
					if(_innerRadius > 0)
					{
						...
						{
							float elev = _elevMin + (j*elevIncr);
							gl.Vertex3f( _centre.x() + _outerRadius*cos(elev)*sin(az),
								_centre.y() + _outerRadius*cos(elev)*cos(az),
								_centre.z() + _outerRadius*sin(elev));
							gl.Vertex3f( _centre.x() + _innerRadius*cos(elev)*sin(az),
								_centre.y() + _innerRadius*cos(elev)*cos(az),
								_centre.z() + _innerRadius*sin(elev));
						}
						gl.End();
...
bool CSphereSegment::Side_computeBound(osg::BoundingBox& bbox,
                                CSphereSegment::SideOrientation orientation,
                                CSphereSegment::BoundaryAngle boundaryAngle) const
{
    bbox.init();
...
void CSphereSegment::Spoke_drawImplementation(osg::State& state, BoundaryAngle azAngle, BoundaryAngle elevAngle) const
{
    if(_drawMask & SPOKES)
    {
        osg::GLBeginEndAdapter& gl = (state.getGLBeginEndAdapter());
...
bool CSphereSegment::Spoke_computeBound(osg::BoundingBox& bbox, BoundaryAngle azAngle, BoundaryAngle elevAngle) const
{
    const float az = (azAngle==MIN?_azMin:_azMax);
    const float elev = (elevAngle==MIN?_elevMin:_elevMax);

    bbox.expandBy(_centre);
    bbox.expandBy(  _centre.x() + _outerRadius*cos(elev)*sin(az),
                    _centre.y() + _outerRadius*cos(elev)*cos(az),
                    _centre.z() + _outerRadius*sin(elev));
...
struct SetLineWidthOnType
{
	SetLineWidthOnType(const std::type_info& t, float linewidth): _t(t), _lineWidth(linewidth) {}
...
protected:
	...
	SetLineWidthOnType& operator = (const SetLineWidthOnType&) { return *this; }
};

void CSphereSegment::setSurfaceColor(const osg::Vec4& c)
{
    _surfaceColor=c;
...
void CSphereSegment::setSpokeColor(const osg::Vec4& c)
{
    _spokeColor=c;
...
void CSphereSegment::setEdgeLineColor(const osg::Vec4& c)
{
    _edgeLineColor=c;
...
void CSphereSegment::setSurfaceLineColor(const osg::Vec4& c)
{
	...
	if(c.w() != 1.0) std::for_each(_drawables.begin(), _drawables.end(), ActivateTransparencyOnType(typeid(SurfaceLine)));
	else std::for_each(_drawables.begin(), _drawables.end(), DeactivateTransparencyOnType(typeid(SurfaceLine)));
}

void CSphereSegment::setSurfaceLineWidth(float linewidth)
{
	_surfaceLineWidth = linewidth;

	if(_surfaceLineWidth > 0.0) std::for_each(_drawables.begin(), _drawables.end(), SetLineWidthOnType(typeid(SurfaceLine), _surfaceLineWidth));
}

void CSphereSegment::setSpokeLineWidth(float linewidth)
{
	_spokeLineWidth = linewidth;

	if(_spokeLineWidth > 0.0) std::for_each(_drawables.begin(), _drawables.end(), SetLineWidthOnType(typeid(Spoke), _spokeLineWidth));
}

void CSphereSegment::setSideColor(const osg::Vec4& c)
{
    _planeColor=c;
...
CSphereSegment::LineList CSphereSegment::computeIntersection(const osg::Matrixd& transform, osg::Node* subgraph)
{
    ...
    for(PolytopeVisitor::HitList::iterator itr = hits.begin();
        ...
        ++itr)
    {
        CSphereSegment::LineList lines = computeIntersection(itr->_matrix, itr->_drawable.get());
...
osg::Node* CSphereSegment::computeIntersectionSubgraph(const osg::Matrixd& transform, osg::Node* subgraph)
{
    OSG_INFO<<"Creating line intersection between sphere segment and subgraph."<<std::endl;
...
namespace SphereSegmentIntersector
{
    ...
    struct TriangleIntersectOperator
    {
        ...
        CSphereSegment::LineList _generatedLines;
        ...
        Edge* addEdge(unsigned int p1, unsigned int p2, Triangle* tri)
        {
            ...
            {
                osg::ref_ptr<Edge> edge = const_cast<Edge*>(itr->get());
                edge->addTriangle(tri);
                return edge;
            }
        }

        CSphereSegment::LineList connectIntersections(EdgeList& hitEdges)
        {
            CSphereSegment::LineList lineList;
        ...
        template<class I>
        CSphereSegment::LineList computeIntersections(I intersector)
        {
        ...
        template<class I>
        void trim(CSphereSegment::LineList& lineList, osg::Vec3Array* sourceLine, I intersector)
        {
        ...
        template<class I>
        void trim(CSphereSegment::LineList& lineList, osg::Vec3Array* sourceLine, I intersector1, I intersector2)
        {
            if (sourceLine->empty()) return;
                ...
                else
                {
                    ...
                    if (first>0)
                    {
                        ...
                        if (possible1 && possible2)
                        {
                            ...
                            double end11 = intersector1.distance(end);
                            double delta1 = (end11-start1);

                            double end21 = intersector2.distance(end);
                            double delta2 = (end21-start2);
        ...
        template<class I>
        void trim(CSphereSegment::LineList& lineList, I intersector)
        {
            CSphereSegment::LineList newLines;

            // collect all the intersecting edges
            for(CSphereSegment::LineList::iterator itr = lineList.begin();
                itr != lineList.end();
                ++itr)
            {
        ...
        template<class I>
        void trim(CSphereSegment::LineList& lineList, I intersector1, I intersector2)
        {
            CSphereSegment::LineList newLines;
        ...
        {
            CSphereSegment::LineList fusedLines;
            CSphereSegment::LineList unfusedLines;

            // first separate the already fused lines from the unfused ones.
            for(CSphereSegment::LineList::iterator itr = _generatedLines.begin();
            ...
            while (unfusedLines.size()>=1)
            {
                ...
                if (linePair._line == linePair._neighbourLine)
                {
                    ...
                    CSphereSegment::LineList::iterator fitr = std::find(unfusedLines.begin(), unfusedLines.end(), line);
                ...
                else
                {
                    ...
                    CSphereSegment::LineList::iterator fitr = std::find(unfusedLines.begin(), unfusedLines.end(), line1);
                    if (fitr != unfusedLines.end())
                    {
                        unfusedLines.erase(fitr);
                    }
...
CSphereSegment::LineList CSphereSegment::computeIntersection(const osg::Matrixd& matrix, osg::Drawable* drawable)
{
    ...
    CSphereSegment::LineList radiusLines = tif.computeIntersections(radiusIntersector);
    CSphereSegment::LineList elevMinLines = tif.computeIntersections(elevMinIntersector);
    CSphereSegment::LineList elevMaxLines = tif.computeIntersections(elevMaxIntersector);
    CSphereSegment::LineList azMinLines;
    CSphereSegment::LineList azMaxLines;
...
osg::Node* CSphereSegment::computeIntersectionSubgraph(const osg::Matrixd& matrix, osg::Drawable* drawable)
{
    CSphereSegment::LineList generatedLines = computeIntersection(matrix, drawable);

    osg::ref_ptr<osg::Geode> geode = new osg::Geode;

    geode->getOrCreateStateSet()->setMode(GL_LIGHTING,osg::StateAttribute::OFF);

    for(CSphereSegment::LineList::iterator itr = generatedLines.begin();
