...
namespace VRSim3D
{
class VRSim3D_EXPORT CSphereSegment: public osg::Geode
{
public:
    ...
    */
    enum DrawMask{
		...
		SURFACELINE =  0x00000020, ///< Draw the line round the edge of the area on the sphere's surface
        ALL =       0x7fffffff  ///< Draw every part of the sphere segment
    };


    /** Default constructor. */
    CSphereSegment():osg::Geode(),
                    _centre(0.0f,0.0f,0.0f), _innerRadius(0.0f), _outerRadius(1.0f),
	...
	@param elevDensity      number of units to divide the 纬线间隔
	@param azDensity      number of units to divide the 经线间隔
    */
    CSphereSegment(const osg::Vec3& centre, float innerradius, float outerradius, float azMin, float azMax,
                    float elevMin, float elevMax, int density, int elevDensity, int azDensity):
        osg::Geode(),
	...
	@param azDensity      number of units to divide the 经线间隔
    */
    CSphereSegment(const osg::Vec3& centre, float innerradius, float outerradius, 
					const osg::Vec3& vec, float azRange,
                    float elevRange, int density, int elevDensity, int azDensity);

    /** Copy constructor */
    CSphereSegment(const CSphereSegment& rhs, const osg::CopyOp& co):
        osg::Geode(rhs,co),
        _centre(rhs._centre), _innerRadius(rhs._innerRadius), _outerRadius(rhs._outerRadius),
        _azMin(rhs._azMin), _azMax(rhs._azMax),
        _elevMin(rhs._elevMin), _elevMax(rhs._elevMax),
        _density(rhs._density),
		_elevDensity(rhs._elevDensity),_azDensity(rhs._azDensity),
    ...
    */
	...
	/** 设置表面纬线间隔 */
	void setElevDensity(int d);

	/** Get the density of the sphere segment */
	int getElevDensity() const;

	/** 设置经线间隔 */
	void setAzDensity(int d);

	/** Get the density of the sphere segment */
	int getAzDensity() const;
    ...
    */
	...
	/** Set the color of the surface line. */
	void setSurfaceLineColor(const osg::Vec4& c);

	/** Get the color of the surface line. */
	const osg::Vec4& getSurfaceLineColor() const { return _surfaceLineColor; }

	void setSurfaceLineWidth(float linewidth);
	float getSurfaceLineWidth() const { return _surfaceLineWidth; }

	void setSpokeLineWidth(float linewidth);
	float getSpokeLineWidth() const { return _spokeLineWidth; }
    ...
    META_Node(osgSim, CSphereSegment);
...
private:
	...
	int _azDensity, _elevDensity;

	float _surfaceLineWidth, _spokeLineWidth;
	...
	osg::Vec4   _surfaceLineColor; //表面网格颜色
